...
class MHA(nn.Module):
    ...
    def forward(self, x: torch.Tensor,
        ...
        if past_key_values is not None:
            attn_output = F.scaled_dot_product_attention(q, k, v, attn_mask = attention_mask).contiguous().transpose(1, 2).reshape(B, L, self.num_attention_heads * self.head_dim)
        else:
            attn_output = F.scaled_dot_product_attention(q, k, v, is_causal=True).contiguous().transpose(1, 2).reshape(B, L, self.num_attention_heads * self.head_dim)
