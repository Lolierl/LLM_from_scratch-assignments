...
def scaled_dot_product_attention(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, attention_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
    # q: (B, H_q, L, head_dim)
    # k: (B, H_kv, L_kv, head_dim)
    # v: (B, H_kv, L_kv, head_dim)
    B, H_q, L, head_dim = q.shape
    Bk, H_kv, L_kv, head_dim_k = k.shape
    Bv, H_kv_v, L_kv_v, head_dim_v = v.shape

    if not (Bk == B and Bv == B):
        raise ValueError("Batch size mismatch between q/k/v")
    if not (head_dim == head_dim_k == head_dim_v):
...
class MHA(nn.Module):
    ...
    def forward(self, x: torch.Tensor,
        ...
        attn_output = scaled_dot_product_attention(q, k, v, attention_mask).contiguous().transpose(1, 2).reshape(B, L, self.num_attention_heads * self.head_dim)
